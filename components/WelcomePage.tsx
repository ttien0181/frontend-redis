                    ...
                    <div className="mt-6 flow-root">
                        <div className="-my-6 divide-y divide-gray-500/10">
                             <div className="space-y-2 py-6">
                                {navItems.map((item) => (
                                    <button key={item.name} onClick={() => { setMobileMenuOpen(false); onNavigate(item.page); }} className={`-mx-3 block rounded-lg px-3 py-2 text-lg font-semibold leading-7 ${activePage === item.page ? 'bg-gray-50 text-teal-600' : 'text-gray-900 hover:bg-gray-50'}`}>
                                        {item.name}
                                    </button>
                                ))}
                            </div>
